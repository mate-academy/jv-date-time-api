...
public class JavaDateTimeApi {
    ...
    public String beforeOrAfter(LocalDate someDate) {
        StringBuilder result = new StringBuilder(String.valueOf(someDate));
        LocalDate now = LocalDate.now();
        return someDate.isAfter(now) ? result.append(" is after ").append(now).toString()
                : (someDate.isBefore(now) ? result.append(" is before ").append(now).toString()
                    : result.append(" is today").toString());
    }
    ...
    public Optional<LocalDate> parseDate(String date) {
        try {
            StringBuilder result = new StringBuilder(date.substring(0, 4))
                    .append("-")
                    .append(date, 4, 6)
                    .append("-")
                    .append(date, 6, 8);
            return Optional.of(LocalDate.parse(result.toString()));
        } catch (RuntimeException exception) {
            return Optional.empty();
        }
    }
