...
public class JavaDateTimeApi {
    ...
    public String beforeOrAfter(LocalDate someDate) {
        ...
        int timeDifferent = LocalDate.now().compareTo(someDate);
        if (timeDifferent < 0) {
            return someDate + " is after " + localTime;
        } else if (timeDifferent > 0) {
            return someDate + " is before " + localTime;
        } else {
    ...
    public Optional<LocalDate> customParseDate(String date) {
        try {
            return Optional.of(LocalDate.parse(date, CUSTUMER_FORMATTER));
        } catch (DateTimeParseException e) {
            return Optional.empty();
        }
    }
    ...
    public String formatDate(LocalDateTime dateTime) {
        return dateTime.format(FORMATE_DATE);
    }
}
