...
public class JavaDateTimeApi {
    ...
    public Optional<Integer> diffBetweenZones(String firstZone, String secondZone) {
        try {
            ZonedDateTime zoneOne = ZonedDateTime.of(LocalDateTime.now(), ZoneId.of(firstZone));
            ZonedDateTime zoneTwo = ZonedDateTime.of(LocalDateTime.now(), ZoneId.of(secondZone));
            return Optional.of((int) Duration.between(zoneOne, zoneTwo).abs().toHours());
        } catch (Exception e) {
            return Optional.empty();
        }
    }
