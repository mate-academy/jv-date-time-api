...
public class JavaDateTimeApi {
    ...
    public String beforeOrAfter(LocalDate someDate) {
        if (LocalDate.now().isAfter(someDate)) {
            return someDate + " is before " + LocalDate.now();
        }
        if (LocalDate.now().isBefore(someDate)) {
    ...
    public Optional<Integer> diffBetweenZones(String firstZone, String secondZone) {
        try {
            ZonedDateTime firstDateTimeZone = ZonedDateTime.now(ZoneId.of(firstZone));
            ZonedDateTime secondDateTimeZone = ZonedDateTime.now(ZoneId.of(secondZone));
            return Optional.of(Math.abs(firstDateTimeZone.getHour()
                    - secondDateTimeZone.getHour()));
