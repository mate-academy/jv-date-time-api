...
public class JavaDateTimeApi {
    ...
    public String beforeOrAfter(LocalDate someDate) {
        LocalDate now = LocalDate.now();
        int n = now.compareTo(someDate);
        return n < 0 ? someDate + " is after " + now
                : n > 0 ? someDate + " is before " + now : someDate + " is today";
    }
    ...
    public Optional<LocalDate> customParseDate(String date) {
        try {
            return Optional.of(LocalDate.parse(date, DD_MMM_YYYY_ENGLISH));
        } catch (Exception e) {
            return Optional.empty();
        }
    }
